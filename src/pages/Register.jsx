...
//   const navigate = useNavigate();
  ...
  const validatePassword = (password) => {
    const errors = [];
    if (password.length < 6) errors.push("Least 6 characters");
    if (!/[0-9]/.test(password))
      errors.push("Least one number (0-9) or a symbol");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Here you would integrate with your AuthProvider
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors flex items-center justify-center disabled:opacity-50"
          >
            {loading ? (
